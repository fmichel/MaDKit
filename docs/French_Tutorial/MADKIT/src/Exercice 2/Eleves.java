...
public class Eleves extends Agent {
	...
	public void activate() {
		...
		requestRole("salle K029", "lycee", "eleve", null);// les agents appartient a la communauté salle K029 ,
	...
	public void live() {
		...
		while (true) {
			...
			if (m != null) {
				if (m.getContent().equals("Perdu"))// si le message est "perdu" il s'est trompé et met fin a son cycle de vie
				{
					if (logger != null)
						logger.info("Je me suis trompe...");
					killAgent(this);

				} else // sinon l'agent a gagné
				{
					if (logger != null)
