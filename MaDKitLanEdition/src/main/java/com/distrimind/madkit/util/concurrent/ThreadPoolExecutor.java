...
public class ThreadPoolExecutor extends AbstractExecutorService {
	...
	private final AtomicInteger workersAdding = new AtomicInteger(0);
	...
	// private final Condition waitingWorkChanged=mainLock.newCondition();
	...
	/**
	 * Workers waiting to be notified
	 */
	volatile int workersWaiting = 0;
	 ...
	 *            newly arriving workers since shutdown began will also eventually
	 *            exit. To guarantee eventual termination, it suffices to always
	 *            interrupt only one idle worker, but shutdown() interrupts all idle
	 *            workers so that redundant workers exit promptly, not waiting for a
	 ...
	 *            boolean indicator is used here rather than a value to ensure reads
	...
	public void setThreadsPriority(int _priority) {
		final ReentrantLock mainLock = this.mainLock;
		mainLock.lock();
		try {
			for (Worker w : workers) {
				w.thread.setPriority(_priority);
			}
		} finally {
			mainLock.unlock();
		}
	...
	private void processWorkerExit(Worker w, boolean completedAbruptly) {
		...
		int workersToAddWhenOtherWaiting = 0;
		mainLock.lock();
		try {
			int c = ctl.get();
		...
		if (workersToAddWhenOtherWaiting > 0) {
			while (workersToAddWhenOtherWaiting-- > 0) {
				workersAdding.addAndGet(-1);
				addWorker();
			}
		...
		/*
		 * int c = ctl.get(); if (runStateLessThan(c, STOP)) { if (!completedAbruptly) {
		 * int min = allowCoreThreadTimeOut ? 0 : corePoolSize; if (min == 0 && !
		 * workQueue.isEmpty()) min = 1; min=workersWaiting>0?workersWaiting+1:min;
		 * 
	...
	private Runnable getTask() {
		...
		for (;;) {
			...
			try {
				Runnable r = (timed && workersWaiting == 0) ? workQueue.poll(keepAliveTime, TimeUnit.NANOSECONDS)
						: workQueue.take();
				if (r != null)
					return r;
				timedOut = true;
	...
	final void runWorker(Worker w) {
		...
		try {
			while (task != null || (task = getTask()) != null /* || workersWaiting>0 */) {
				if (task != null) {
					w.lock();
				 ...
				 * else {
				 * 
				 * mainLock.lock(); try { waitingWorkChanged.await(1000, TimeUnit.MILLISECONDS);
	...
	protected Worker getWorker(Thread wt) {
		mainLock.lock();
		try {
			for (Worker w : workers) {
				if (wt.equals(w.thread)) {
					return w;
				}
			}
		} finally {
	...
	private boolean addWorker(Runnable command) {
		...
		if (wc < corePoolSize)
			return addWorker(command, true);
		else
			return addWorker(command, false);
	}
	...
	/*
	 * void notifyWaitingWorkers() {
	 * 
	 * mainLock.lock();
	 * 
	 * try { waitingWorkChanged.signalAll(); } finally { mainLock.unlock(); } }
	 */

	public <V> V takeToBlockingQueue(BlockingDeque<V> toTake) throws InterruptedException {
		mainLock.lock();
		if (getWorker(Thread.currentThread()) != null) {
			try {
				++workersWaiting;
				if (workersWaiting >= getPoolSize()) {
					addWorker();
				}
				/*
				 * else notifyWaitingWorkers();
				 */
	...
	<V> boolean futureGet(Future<V> future, AtomicReference<V> result) throws InterruptedException, ExecutionException {
		mainLock.lock();
		if (getWorker(Thread.currentThread()) != null) {
			try {
				++workersWaiting;
	...
	<V> boolean futureGet(Future<V> future, AtomicReference<V> result, long _timeout, TimeUnit _unit)
		...
		if (getWorker(Thread.currentThread()) != null) {
			try {
				...
				mainLock.unlock();
				result.set(future.get(_timeout, _unit));
			} finally {
				mainLock.lock();
				--workersWaiting;
				// notifyWaitingWorkers();
	...
	public boolean sleep(long millis) throws InterruptedException {
		long endTimeMilli = System.currentTimeMillis() + millis;
		if (getWorker(Thread.currentThread()) != null) {
			++workersWaiting;
			...
			try {
				...
				long timeout = endTimeMilli - System.currentTimeMillis();
				if (timeout > 0)
	 ...
	 * @throws IllegalArgumentException
	 *             if value is {@code true} and the current keep-alive time is not
	 *             greater than zero
	 *
	 * @since 1.6
	 */
	 ...
	 *            the new maximum
	 * @throws IllegalArgumentException
	 *             if the new maximum is less than or equal to zero, or less than
	 ...
	 *
	 * @param time
	 *            the time to wait. A time value of zero will cause excess threads
