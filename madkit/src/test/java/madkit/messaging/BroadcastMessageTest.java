	...
	public void returnSuccess() {
		launchTestedAgent(new Agent() {
			protected void onActivation() {
				...
				threadAssertEquals(SUCCESS, broadcast(new StringMessage("test"), getAgentsWithRole(COMMUNITY, GROUP, ROLE)));
				threadAssertNotNull(target.nextMessage());
				threadAssertEquals(ROLE, m.getReceiver().getRole());

				// With role
				threadAssertEquals(SUCCESS, broadcastWithRole(new Message(), getAgentsWithRole(COMMUNITY, GROUP, ROLE), ROLE));
	...
	public void returnNotInGroup() {
		launchTestedAgent(new Agent() {
			protected void onActivation() {
				...
				// With role
				threadAssertEquals(NOT_IN_GROUP, broadcastWithRole(new Message(), getAgentsWithRole(COMMUNITY, GROUP, ROLE), ROLE));
			}
		});
	}
