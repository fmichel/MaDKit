...
public class ScheduledThreadPoolExecutor extends ThreadPoolExecutor implements ScheduledExecutorService {
	 ...
	 *
	 * 1. Using a custom task type, ScheduledFutureTask for tasks, even those that
	 * don't require scheduling (i.e., those submitted using ExecutorService
	 * execute, not ScheduledExecutorService methods) which are treated as delayed
	 * tasks with a delay of zero.
	 *
	 * 2. Using a custom queue (DelayedWorkQueue), a variant of unbounded
	 * DelayQueue. The lack of capacity constraint and the fact that corePoolSize
	 * and maximumPoolSize are effectively identical simplifies some execution
	 * mechanics (see delayedExecute) compared to ThreadPoolExecutor.
	 *
	 * 3. Supporting optional run-after-shutdown parameters, which leads to
	 * overrides of shutdown methods to remove and cancel tasks that should NOT be
	 * run after shutdown, as well as different recheck logic when task
	 * (re)submission overlaps with a shutdown.
	 *
	 * 4. Task decoration methods to allow interception and instrumentation, which
	 * are needed because subclasses cannot otherwise override submit methods to get
	 * this effect. These don't have any impact on pool control logic though.
	 */
	 ...
	 */
	protected volatile boolean removeOnCancel = false;

	/**
	 * Sequence number to break scheduling ties, and in turn to guarantee FIFO order
	 * among tied entries.
	 */
	protected static final AtomicLong sequencer = new AtomicLong(0);
		...
		if (!keepDelayed && !keepPeriodic) {
			...
			for (Object e : q.toArray()) {
				if (e instanceof RunnableScheduledFuture) {
					...
					if ((t.isPeriodic() ? !keepPeriodic : !keepDelayed) || t.isCancelled()) { // also remove if already
																								// cancelled
						if (q.remove(t))
							t.cancel(false);
					}
	...
	/**
	 * Constrains the values of all delays in the queue to be within Long.MAX_VALUE
	 * of each other, to avoid overflow in compareTo. This may occur if a task is
	 ...
	 * when this executor has been {@code shutdown}. In this case, these tasks will
	 * only terminate upon {@code shutdownNow} or after setting the policy to
	 * {@code false} when already shutdown. This value is by default {@code false}.
	 *
	 * @param value
	 ...
	 * executor has been {@code shutdown}. In this case, these tasks will only
	 * terminate upon {@code shutdownNow}, or after setting the policy to
	 ...
	 * the work queue at time of cancellation. This value is by default
	 * {@code false}.
	 *
	 * @param value
	 ...
	 *
	 * <p>
	 * If the {@code ExecuteExistingDelayedTasksAfterShutdownPolicy} has been set
	 * {@code false}, existing delayed tasks whose delays have not yet elapsed are
	 * cancelled. And unless the {@code
	 * ContinueExistingPeriodicTasksAfterShutdownPolicy} has been set {@code true},
	 ...
	 * never terminate.
	 *
	 * @return list of tasks that never commenced execution. Each element of this
	 *         list is a {@link ScheduledFuture}, including those tasks submitted
	 *         using {@code execute}, which are for scheduling purposes used as the
	 ...
	 * zero-delay {@code ScheduledFuture}. Iteration over this queue is <em>not</em>
	...
	static class DelayedWorkQueue extends AbstractQueue<Runnable> implements BlockingQueue<Runnable> {
		 ...
		 * DelayQueue and PriorityQueue, except that every ScheduledFutureTask also
		 * records its index into the heap array. This eliminates the need to find a
		 * task upon cancellation, greatly speeding up removal (down from O(n) to O(log
		 * n)), and reducing garbage retention that would otherwise occur by waiting for
		 * the element to rise to top before clearing. But because the queue may also
		 * hold RunnableScheduledFutures that are not ScheduledFutureTasks, we are not
		 * guaranteed to have such indices available, in which case we fall back to
		 * linear search. (We expect that most tasks will not be decorated, and that the
		 * faster cases will be much more common.)
		 *
		 * All heap operations must record index changes -- mainly within siftUp and
		 * siftDown. Upon removal, a task's heapIndex is set to -1. Note that
		 * ScheduledFutureTasks can appear at most once in the queue (this need not be
		 * true for other kinds of tasks or work queues), so are uniquely identified by
		 * heapIndex.
		 */
		...
		/**
		 * Sift element added at bottom up to its heap-ordered spot. Call only when
		 * holding lock.
		 */
		...
		/**
		 * Return and remove first element only if it is expired. Used only by drainTo.
